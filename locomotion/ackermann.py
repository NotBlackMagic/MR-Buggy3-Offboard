...
class AckermannOdometry:
	...
	def rc_command_throttle_to_speed(self, throttle: float):
		...
		# Scale RC command to real speed in m/s using simple linear function: y = s[0] + s[1] * x
	...
	def speed_to_rc_command_throttle(self, speed: float):
		...
		# Scale real speed in m/s to RC command using simple linear function
	...
	def rc_command_steering_to_angle(self, steer: float):
		...
		# Scale RC command to real angle in rad using simple linear function: y = s[0] + s[1] * x
	...
	def angle_to_rc_command_steering(self, angle: float):
		...
		# Scale real angle in rad to RC command using simple linear function
	...
	def update_using_rc_command(self, steer: float, speed: float):
		...
		# Update linear twist values, based only on RC command
